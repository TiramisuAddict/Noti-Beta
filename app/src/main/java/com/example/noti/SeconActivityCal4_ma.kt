...
class SeconActivityCal4_ma  : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_secon_cal4_ma)
        ...
        btn.setOnClickListener {
            if (!checkBoxSport.isChecked) { //Exempt Sport #--False--#
                ...
                startActivity(
                    Intent(this@SeconActivityCal4_ma, ResActivity::class.java)
                ...
                val CalculationResult =
                    ((n1res.text.toString().toFloat() * 1)
                            ...
                            + (n7res.text.toString().toFloat() * 1)
                            + (n8res.text.toString().toFloat() * 1)
                            + (n9res.text.toString().toFloat() * 1) ) / 15
                val R_CalculationResult = BigDecimal(CalculationResult.toFloat().toDouble()).setScale(2, RoundingMode.HALF_EVEN)
                val FinalResult = R_CalculationResult.toString()
